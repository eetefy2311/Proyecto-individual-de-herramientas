...
ggsave(here("res", "blanqueamiento_tiempo.pdf"), plt, width = 6, height = 4)
```
...
ggsave(here("res", "blanqueamiento_colores.pdf"), plt1, width = 6, height = 4)
...
ggsave(here("res", "pastel.pdf"), plt2, width = 6, height = 4)
...
ggsave(here("res", "distancia.pdf"), plt3, width = 6, height = 4)
```
Análisis
...
ggsave(here("res", "temp_mapa.pdf"), plt4, width = 6, height = 4)
...
ggsave(here("res", "anomalias.pdf"), plt5, width = 6, height = 4)
```
...
c(RMSE = rmse, R2 = r2)
plt6 <- ggplot(data.frame(obs = test$Percent_Bleaching, pred = pred.test),
       aes(x = obs, y = pred)) +
...
ggsave(here("res", "modelo.pdf"), plt6, width = 6, height = 4)
```
